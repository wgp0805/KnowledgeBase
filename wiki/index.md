...
- [[matt-skills-boundary-judgment]] — Matt Skills 边界判断：TDD 无 seam 场景跳过规则 / implement vs tdd / implement vs 直接让 AI 干活 / 通用速查表
...
- [[AI产品]] — AI产品方法论：先回答谁付钱解决什么问题，功能无壁垒业务数据循环才有
...
## Sources (2026-09-14 批量 ingest - 29 篇文章)
- [[摘要-领导商量其实是通知]] — 职场沟通：领导"商量"实为通知，识别权力语言与应对策略
- [[摘要-产品人失守的三道门]] — 产品经理失守路径：伪需求/甩锅/附和老板，三道门一步步退让
- [[摘要-elk日志系统搭建教程]] — ELK 日志系统从零搭建：Filebeat + Logstash + ES + Kibana 保姆级教程
- [[摘要-gpt-image-2.5做图工作流]] — GPT Image 2.5 从"生图"到"做图"工作流体验
- [[摘要-ai大厂抢输入法入口]] — 输入法成为 AI 大厂争夺的下一个超级入口
- [[摘要-agentmemory开源记忆层]] — AgentMemory 开源记忆层解决 AI 编程助手"失忆"问题
- [[摘要-claude-code-8问最狠难题怎么破]] — Claude Code 8 个最狠难题排障指南
- [[摘要-ai长剧的尴尬降本容易赚钱难]] — AI 长剧降本容易赚钱难的商业模式困境
- [[摘要-模型发布比快递还快AI产品经理该怎么选]] — 模型选型四点建议：AI 产品经理在模型快速迭代中如何选
- [[摘要-小米的营销真的有点可怕]] — 小米营销方法论与需求分类解析
- [[摘要-向量检索引擎从embedding到相似度匹配的工程实践]] — 向量检索引擎工程实践：从 Embedding 到相似度匹配
- [[摘要-美国版豆包又更新了]] — Gemini Desktop 与 Antigravity 撞车，"美国版豆包"又更新了
- [[摘要-gavin-baker-所有人都在等ai泡沫但算力还是不够]] — Gavin Baker：所有人都在等 AI 泡沫，但算力还是不够
- [[摘要-codex差点把我搞疯]] — Codex 部署踩坑实录
- [[摘要-4万弹珠机项目程序员ai提效20天交付]] — 4 万弹珠机项目，程序员 AI 提效 20 天交付
- [[摘要-prompt-skill-做减法]] — 模型越聪明，Prompt 和 Skill 越要做减法
- [[摘要-ai产品商业模式拆解]] — 拆解 AI 产品商业模式：别迷信技术，这才是真相
- [[摘要-openai产品设计负责人对话]] — 对话 OpenAI 产品设计负责人：只会往聊天框里堆功能的产品设计师只是在假装做 AI 产品
- [[摘要-ai客服项目复盘72%接管率]] — 从 0 到 72% 接管率：一个 AI 客服项目的完整复盘
- [[摘要-ai产业升级与普通人成长困境]] — AI、产业升级与普通人的成长困境
- [[摘要-阿里不推荐keyset遍历hashmap]] — 为什么阿里不推荐使用 keySet() 遍历 HashMap
- [[摘要-短剧app广告变现策略]] — 短剧 APP 广告变现策略与商业模式
- [[摘要-用扣子搭ai客服接单教程]] — 用扣子搭 AI 客服从 0 到 1 接单保姆级教程
- [[摘要-产品经理行业赛道全景与能力差异]] — 同样叫产品经理，换个行业为什么面试难度和薪资差这么多
- [[摘要-ai-native组织运行方式]] — 当 Agent 成为正式参与者，AI Native 组织如何运行
- [[摘要-sqlite嵌入式数据库引擎]] — SQLite 3.x 全球部署最广泛的零配置嵌入式 ACID 关系型数据库引擎
- [[摘要-ai文案工具架构复盘]] — 30 秒出一条多渠道电商文案：AI 文案工具架构复盘
- [[摘要-10亿数据快速插入mysql方案]] — 阿里终面：10 亿数据如何快速插入 MySQL
- [[摘要-字节开源openviking-agent上下文数据库]] — 字节开源 OpenViking Agent 上下文数据库

## Entities (2026-09-14 新增 ingest)
- [[Filebeat]] — Elastic 开源轻量级日志采集器，ELK 第一环
- [[AgentMemory]] — 开源 AI Agent 记忆层项目，解决编程助手"失忆"问题
- [[GavinBaker]] — 知名科技投资人/分析师，AI 算力需求乐观派
- [[HashMap]] — Java 核心数据结构，基于哈希表的键值对存储

## Concepts (2026-09-14 新增 ingest)
- [[向量检索引擎]] — 基于 Embedding 向量进行相似度匹配的检索引擎
- [[ELK日志系统]] — Elasticsearch + Logstash + Kibana 组合的日志采集分析系统
- [[短剧广告变现]] — 短剧 APP 通过广告投放实现流量变现的商业模式
- [[AI客服]] — 基于 AI 的自动客服系统，核心指标为接管率
- [[AI文案工具]] — 基于 AI 自动生成营销文案的工具
- [[批量数据插入]] — 大规模数据高效写入数据库的技术方案
- [[上下文数据库]] — 火山引擎 OpenViking 提出的虚拟文件系统，统一知识库/记忆/技能
...
## 2026-09-15 lint 修复 - 新建 104 页

### Concepts
- [[办公Agent]] — 面向办公场景的 Agent 形态：文档生成、数据清洗、会议总结、日程协作；豆包工作、千问办公、W…
- [[本地部署]] — 把模型部署在自有机器而非云端的做法，涉及 GGUF 量化、显存估算、Swap 配置与离线数据闭环
- [[变更治理]] — 对配置与规则的变更做申请、评审、灰度、回滚的全流程管理，是企业配置中心的成熟度标志
- [[不确定性上浮]] — 与「确定性下沉」相对的设计取向：让不确定性越靠上、越接近人决策的层，底座保持确定可测
- [[产品经理角色]] — 产品经理的职责边界与定位：定义问题、定义成功标准、协调资源；也是「领导商量其实是通知」等职场议题…
- [[传统RAG]] — 原始 RAG 范式：切块、嵌入、向量库、检索、拼接上下文、生成；检索过程黑盒、用完即弃，可审计性…
- [[创业方法论]] — 创业从 0 到 1 的方法体系，核心争论是价值创造还是价值捕获、是抢存量还是造增量
- [[代理认证]] — 通过代理访问目标服务时的身份认证机制（HTTP 代理 Basic/NTLM、SOCKS5 用户名…
- [[代码能力]] — AI 模型完成编程任务的能力维度：代码生成、理解、调试、重构与端到端交付；前沿模型竞争的核心指标…
- [[代码审查]] — 通过人工或 AI 评审他人代码来保障质量、传递知识、约束规范的过程；Open Code Revi…
- [[递归式自我改进]] — AI 系统用自身产出参与自身训练或优化的循环（RSI），GPT-6 训练后期被工程师称为「递归式…
- [[关键词研究]] — 找出目标用户真实搜索词的调研过程，是 SEO 与内容策略的起点；工具如 Ahrefs、Semru…
- [[国产芯片]] — 中国自主 AI 训练与推理芯片（华为昇腾、寒武纪、海光等），是超大规模推理集群的国产化路径，也是…
- [[过拟合]] — 模型在训练集上表现好但泛化差的现象；LoRA 微调小数据集时的高频风险，需用验证集与早停控制
- [[激素监测]] — 通过可穿戴传感器持续采集生理信号并反推激素变化（如经前期波动）的技术方向，Clair 女性健康手…
- [[可穿戴设备]] — 佩戴在身上的传感终端形态（手环、贴片、戒指），是健康数据的持续采集入口
- [[可观察检索]] — 让检索过程可被观察、评估与调试的检索范式，与黑盒检索相对，是 RAG 可审计化的方向
- [[幂等性]] — 同一操作执行一次与多次效果相同；消息重试、表单重复提交、分布式协调都必须解决的性质
- [[内容版本管理]] — 对内容稿件做版本留痕、比对与回滚的能力，是 CMS 在 AI 参与创作后的必要约束
- [[配置复用]] — 让同一份配置在多个环境与服务间共享并保持一致的机制，与配置作用域共同决定配置的可维护性
- [[配置中心]] — 集中管理应用配置并提供动态下发、权限、审计与灰度能力的平台（Nacos、Apollo、Consu…
- [[确定性工程]] — 把不确定的 LLM 输出用确定性流程（校验、规则、编译期检查）约束为可交付结果的工程方法
- [[人效管理]] — 以单位人力产出为核心指标的管理方法；AI 时代的度量正从人均营收转向 Token 效率等新型指标
- [[认知偏差]] — 人类系统性偏离理性判断的思维倾向（确认偏误、锚定效应等）；也是理解 AI 讨好偏见的对照概念
- [[沙箱]] — 隔离执行环境：为 Agent 工具调用提供受限的文件系统、进程与网络边界，即使出错也不影响宿主系…
- [[审核流程]] — 内容从草稿到发布的评审路径（自审、他审、合规审），常与状态机一起设计
- [[数据分析Agent]] — 以 Agent 形态执行数据分析任务（取数、清洗、口径核对、出图），核心难点是口径标准而非模型能…
- [[数据治理]] — 对数据的质量、口径、权限、生命周期做统一管理的体系；AI 项目里数据清洗规则必须可回滚、可审计
- [[推理系统]] — 承接模型上线后请求的整套软件栈（调度、批处理、KV 管理、量化、多卡切分），决定单位 Token…
- [[维度文档]] — 明确每个分析维度（城市、渠道、时间粒度）定义与口径的文档，是数据分析 Agent 对齐口径的依据
- [[物联网]] — 通过传感器与网络让物理设备联网交换数据的技术体系（NB-IoT、MQTT、LoRa），GPS 定…
- [[向量数据库]] — 以高维向量为主键并提供 ANN 近似最近邻检索的数据库（Faiss、Milvus、Chroma、…
- [[虚拟文件系统]] — 用统一文件系统接口封装异构数据源（知识库、记忆、技能）的抽象层，OpenViking 与 LLM…
- [[语音AI]] — 语音识别与合成能力在 AI 产品中的集成形态，女性健康手环等产品用它做语音交互
- [[灾难性遗忘]] — 模型在继续训练后丢失原有能力的现象，是 LoRA/SFT 微调的核心风险之一
- [[张量并行]] — 把单层权重矩阵按维度切分到多卡并行计算的推理与训练策略，与流水线并行互补，用于单步加速
- [[长上下文]] — 模型支持超长输入窗口的能力（128K 到 1M token），带来窗口利用、KV Cache 成…
- [[知识库]] — 组织或个人结构化沉淀的知识集合；AI 时代的核心问题是「怎么把散落的知识喂给 Agent」，而不…
- [[知识库数据资产]] — 把知识库当作可复用、可估值的数据资产来运营，与「一次性检索工具」相对
- [[职场弱势]] — 组织中信息不对称导致的话语权弱势状态（如「领导商量其实是通知」），是职场沟通技巧的讨论起点
- [[状态机]] — 用有限状态与受控迁移描述对象生命周期的建模方法；CMS 审核流、共识算法、任务调度都靠它
- [[组织变革]] — 组织结构、流程与人员配置的系统性调整；Agentic Organization 背景下表现为拆墙…
- [[组织数据资产]] — 组织内部沉淀的结构化数据（客户、交易、行为、知识库）作为核心资产的视角；飞书与豆包打通后的关键议…
- [[最终一致性]] — 放弃强一致、允许短暂不一致以换取可用性与性能的分布式一致性模型；Spring 事件与消息队列场景…
- [[ABAC]] — 基于属性的访问控制：用主体、资源、环境属性组合表达式做授权决策，比 RBAC 更细但更难治理
- [[AGI]] — Artificial General Intelligence：具备通用人类水平智能的 AI；B…
- [[AI安全]] — 确保 AI 系统不被滥用、不造成危害的工程与治理体系；GPT-6 是首个触到网络安全 Criti…
- [[AI对齐]] — 让 AI 目标与人类意图保持一致的研究方向；「智能的进步不保证对齐的进步」是 Pachocki …
- [[AI内容生产]] — 用 AI 生成并批量产出内容（文案、图片、视频）的生产方式，倒逼 CMS 重做版本、审核与数据回…
- [[AI视频生成]] — 用扩散或 DiT 模型从文本、图片生成视频的技术与产品形态，2026 年赛道竞争激烈
- [[AI讨好偏见]] — 模型倾向迎合用户观点的偏差（Sycophancy），会降低事实性；对抗方法是钢人论证与双向对称对…
- [[B端Agent]] — 面向企业客户的 Agent 产品分层：规则、Workflow、LLM 节点、Agent；确定性下…
- [[ChatML]] — Chat 模型的对话标记格式（system、human、gpt 轮次），SFT 微调数据集的常用…
- [[DPO]] — Direct Preference Optimization：绕开显式奖励模型，直接在偏好对上优…
- [[FDA510k]] — 美国 FDA 的上市前通知程序，二类医疗器械通用审批路径；女性健康手环等消费健康设备需评估是否适…
- [[FDE]] — Forward Deployed Engineer：深入客户现场、把通用能力工程化为可交付产品的…
- [[FOR压缩]] — Frequent OR 编码：倒排索引 Posting List 的压缩方案，用 WAND/BM…
- [[GroundTruth]] — 人工确认的标准答案集合，用于评估数据分析 Agent 与 RAG 的准确性；口径管理的基础设施
- [[KV缓存压缩]] — 降低 Transformer 推理时 KV Cache 显存占用的技术族：量化、共享、驱逐、滑动…
- [[LayerSplit]] — 把模型层按 Layer 切分到多设备执行的技术，与张量并行、流水线并行共同构成大模型分布式推理的…
- [[Marketplace]] — 双边或多边市场：平台同时服务供给方与需求方，价值随两侧规模共同增长（网络效应），核心是撮合而非卖…
- [[Paxos]] — 经典分布式共识算法（Lamport 1990）：通过 Prepare/Promise/Accep…
- [[PermissionRequest]] — Claude Code Hooks 事件：工具调用请求权限时触发，可拦截或自动批准
- [[PostToolUse]] — Claude Code Hooks 事件：工具调用成功返回后触发，可用于校验输出、注入上下文
- [[PreCompact]] — Claude Code Hooks 事件：上下文压缩前触发，用于在压缩前抢救关键信息
- [[PreToolUse]] — Claude Code Hooks 事件：工具调用前触发，是「防危险命令」的第一道闸门
- [[QLoRA]] — 量化版 LoRA：把基座模型量化到 4-bit 后只训练低秩适配器，显著降低微调显存门槛
- [[Raft]] — 可理解的分布式共识算法（Ongaro & Ousterhout 2014），拆成领导者选举（随机…
- [[ReBAC]] — 基于关系的访问控制：沿「用户到组到资源」的关系图判定权限，适合多租户与共享资源场景
- [[Resume模式]] — 从已保存状态恢复执行的模式（Agent 会话、工作流、终端会话），减少重复上下文开销
- [[RLVR]] — Reinforcement Learning with Verifiable Rewards：用…
- [[SessionStart]] — Claude Code Hooks 事件：新会话启动时触发，可注入项目上下文、恢复记忆
- [[SFTP传输]] — 基于 SSH 的文件传输协议，比 SCP 更安全可控；FinalShell、Xshell 等运维…
- [[Stop]] — Claude Code Hooks 事件：Agent 停止响应时触发，可检查是否真的完成、追加总…
- [[TD残差]] — Temporal Difference 误差：当前估值与「下一步估值加奖励」之差，是 Actor…
- [[TermDictionary]] — 倒排索引中间层：存储词项并指向 Posting List 的位置，通常配合 FST 做压缩与二分…
- [[VSCode扩展]] — 在 VS Code 内以插件形态提供能力的载体；AIAgentsTerminal 这类「一键启动…
- [[OpaqueRecurrence]] — 不透明递归（opaque recurrence）：指大模型在推理时把一部分本来可以监控的中间推理…

### Entities
- [[阿里巴巴]] — 中国电商与云计算集团，旗下有阿里云、通义千问、ModelScope，并开源了 Open Code…
- [[阿里云]] — 阿里巴巴的云计算业务，提供通义千问、ModelScope、DashScope API 与算力，是…
- [[机器人]] — 具备感知与执行能力的物理自动化设备；AI 泡沫讨论中常被拿来类比自动驾驶
- [[即梦]] — 字节跳动 AI 视频与图像生成产品，主打中文创作者场景
- [[可灵]] — 快手 AI 视频生成产品，以高质量长视频生成见长
- [[千问办公]] — 阿里通义千问的办公 Agent 产品，把大模型能力嵌入文档、表格与协作流，与豆包工作、飞书构成办…
- [[自动驾驶]] — 车辆自主驾驶技术；与机器人一起被视为 AI 落地的长期赛道，也是 AI 泡沫争论的反例论据
- [[Autowired]] — Spring 依赖注入注解：按类型自动装配 Bean，是动态切换实现类、Smart DI 等方案…
- [[ClaudeFable5]] — Anthropic Claude 的旗舰模型档位，常被拿来做 GPT-6 Astra 的能力对标
- [[ComfyUI]] — 基于节点的 Stable Diffusion 工作流工具，用可视化 DAG 编排采样与后处理，适…
- [[GoogleADK]] — Google Agent Development Kit：Google 面向 Agent 开发的…
- [[Hermes]] — Nous Research 的开源模型与编程 harness 系列，以工具调用与自我进化见长
- [[Jackson3TypeHandler]] — Jackson 3 对应的 MyBatis TypeHandler，负责 JSON 字段与 Ja…
- [[ModelScope]] — 阿里达摩院开源的模型社区与框架，提供模型下载、微调与部署工具链
- [[OFD]] — 国产版式文档格式（Open Fixed-layout Document），与 PDF 并列；kk…
- [[OiiOii]] — AI 动画与视频生成产品，常被用来与 MiniMax Design、Runway 做横评
- [[PDF.js]] — Mozilla 开源的浏览器端 PDF 渲染库，是 kkFileView 在线预览 PDF 的底…
- [[Pika]] — AI 视频生成公司，主打图生视频、文生视频与视频编辑
- [[Python]] — 通用解释型编程语言，AI 生态的主导语言；requests、aiohttp、httpx 等库的 …
- [[Qwen2]] — 阿里通义千问第二代模型系列，常被用作 GGUF 量化与本地部署的基座模型
- [[RoaringBitmaps]] — 面向整型集合的高效压缩数据结构，倒排索引 Posting List 常用它替代传统 FOR 编码
- [[Runway]] — AI 视频生成公司，Gen 系列主打影视级生成与编辑，是视频生成赛道头部玩家
- [[S4HANA]] — SAP 的 HANA 内存计算企业套件，是财务数字化三层架构中报表合并与集团科目的典型实现
- [[Stargate]] — OpenAI 位于得州的超大规模训练基地，GPT-6 预训练动用了超 10 万块 GPU
- [[Transformers]] — Hugging Face 的模型训练与推理框架，是 LoRA-SFT 微调的标准工具链
- [[ZonedDateTime]] — java.time 中携带时区信息的日期时间类型，跨时区处理应优先于 Date 与 LocalD…


## 2026-09-15 lint 修复 - 补登记 31 页

### Concepts
- [[动态映射]] — 动态映射（Dynamic Mapping）是 Elasticsearch 在未显式定义 Mapp…
- [[多Agent共享记忆]] — 多 Agent 共享记忆是指通过统一的上下文数据库（如 OpenViking），让不同 Agen…
- [[口径管理]] — 口径（Ground Truth）是一个指标的计算规则——分子是什么，分母是什么，哪些场景计入，哪…
- [[配置作用域]] — 配置作用域回答"规则在哪里有效"，不是一棵树，而是一组解析条件。最终配置是系统在"空间 × 类型…
- [[确定性下沉]] — 确定性下沉是 Agent 产品的核心架构原则：能写成规则的不要交给大模型，能固化成 Workfl…
- [[上下文分层]] — 上下文分层是 OpenViking 的核心检索机制，为内容生成三层上下文：L0摘要层→L1概览层…
- [[Agent扩展层级]] — Agent 扩展层级是指对 AI Agent 系统进行能力扩展时的不同深度等级。不同层级修改的对…
- [[ESKeyword]] — keyword 是 Elasticsearch 中用于精确匹配、排序和聚合的字符串类型，不会被分…
- [[ESMapping]] — Mapping 是 Elasticsearch 中定义索引字段名、数据类型及相关配置的过程，类似…
- [[ESNested]] — nested 是 Elasticsearch 中用于存储嵌套对象的特殊数据类型，能够保持嵌套对象…
- [[ESObject]] — object 是 Elasticsearch 中用于存储 JSON 对象的默认数据类型，支持嵌套…
- [[ESText]] — text 是 Elasticsearch 中用于全文检索的字符串类型，会被分词器处理，将文本拆分…
- [[ESType]] — Type 是 Elasticsearch 7.0 之前用于在索引内逻辑分组文档的概念，类似于关系…
- [[GraphWorkflow]] — Graph Workflow 是 Google ADK 2.0 引入的工作流编排模式，将 Age…
- [[LLM Loop]] — LLM Loop（大模型循环）是 AI Agent 的核心运行机制：大模型在一个循环中不断思考、…
- [[PostingList]] — Posting List（倒排列表）是倒排索引三层结构的底层，存储每个词条对应的文档信息列表，包…
- [[RepairLoop]] — Repair Loop（自动循环修复）是 Agent 的自我纠错机制：Agent 修改代码后进入…
- [[scaled_float]] — scaled_float 是 Elasticsearch 中用于存储精确数值的浮点类型，通过指定…
- [[AI产品]] — 以模型能力为产品核心链路的一类产物：模型不再是产品的附属功能，而是定价、交付与迭代节奏的决定项。…

### Entities
- [[人人都是产品经理]] — 中国产品人社区与内容平台（woshipm.com），聚集产品经理与运营从业者，既有产品方法论与增…
- [[Ahrefs]] — 专业的SEO工具平台，提供关键词研究、竞品分析、内容差距挖掘等功能。
- [[AIAgentsTerminal]] — AI Agents Terminal 是一个 VS Code 扩展，把本机已安装的 AI CLI…
- [[ClairHealth]] — Clair Health 是一家专注于女性健康的可穿戴设备公司，通过多模态生物传感和机器学习实现…
- [[OpenCodeReview]] — Open Code Review 是阿里巴巴开源的 AI 代码审查 Agent，源自内部官方 A…
- [[OpenViking]] — OpenViking 是火山引擎开源的面向 AI Agent 的上下文数据库，以虚拟文件系统统一…
- [[Pi (coding harness)]] — Pi（pi.dev）是 Earendil Inc. 出品的**极简终端编程 Agent 基座**…
- [[Semrush]] — 专业的SEO工具平台，提供关键词研究、竞品分析、内容差距挖掘等功能。
- [[SpringSmartDI]] — `spring-smart-di` 是一个扩展 Spring `@Autowired` 注入逻辑…

### Sources
- [[摘要-pi-agent-study-01-pi-overview]] — 本篇是 Pi (pi.dev) 系统学习的第一课笔记，主要讲解 Pi 的定位、核心理念以及与其他…
- [[摘要-pi-study-02-architecture]] — 本篇是 Pi (pi.dev) 系统学习的第二课笔记，详细讲解 Pi 的整体架构。Pi 采用"小…
- [[摘要-spring-smart-di-动态切换实现类]] — `spring-smart-di` 是对 Spring `@Autowired` 注解的创新性扩…
