...
| 维度 | 提示词（Prompt） | Skill | Loop（开发循环） |
|------|-----------------|-------|-----------------|
| **本质** | 一次性指令，说完就没了 | 常驻的操作说明书（SKILL.md） | 三文件驱动的逐步迭代开发 |
| **比喻** | 临时喊一嗓子"帮我倒杯水" | 冰箱上贴的纸条"客人来了先倒水" | 带实习生逐项完成任务，每步确认 |
| **生命周期** | 单次对话有效 | 常驻，遇到相关任务自动触发 | 持续运行直到项目完成 |
...
### ⚠️ 重要区分
本指南中的"循环"指的是**三文件驱动的开发循环**（CLAUDE.md + STATE.md + SKILL.md），**不是** Claude Code 内置的 `/loop` 定时命令。区别如下：

| | 内置 `/loop` 命令 | 三文件开发循环 |
|---|---|---|
| **用途** | 定时反复执行同一任务 | 逐步构建项目，每步不同 |
| **触发方式** | `/loop 5m python todo.py list` | 对 AI 说"开始开发循环" |
| **典型场景** | 每5分钟检查一次部署状态 | 逐轮开发一个完整项目 |

### 目标
5 分钟内体验开发循环的完整逻辑——AI 自动逐轮构建，每轮只做一件事，你确认后继续。
...
```
开始开发循环，按照 CLAUDE.md 和 STATE.md 的进度，逐步构建这个命令行备忘录工具。每完成一步更新 STATE.md 并问我是否继续。
```
...
| 你想做什么 | 输入 |
|-----------|------|
| 启动开发循环 | 对 AI 说"开始开发循环，按 CLAUDE.md 和 STATE.md 推进" |
| 指定本轮范围 | 对 AI 说"这次只做前端部分，暂不碰后端" |
| 设定目标 | 对 AI 说"完成所有 CRUD API 且测试通过之前不要停" |
| 查看进度 | 对 AI 说"当前进度如何？"或 `/read STATE.md` |
| 暂停循环 | 直接说"暂停"或 Ctrl+C |
| 切换方向 | 对 AI 说"先做状态流转，优先级管理后面再做" |
| 无人值守 | 对 AI 说"自动继续，不需要我确认，做完汇报就行" |

### 附：内置 `/loop` 定时命令（仅用于定时任务）

Claude Code 内置的 `/loop` 命令用于**定时重复执行同一任务**，语法如下：

| 命令 | 作用 |
|------|------|
| `/loop 5m python todo.py list` | 每 5 分钟运行一次 list 命令 |
| `/loop 30m 检查部署状态` | 每 30 分钟检查一次部署 |
| `/loop 1h /review` | 每小时运行一次 review 命令 |

它与本文介绍的三文件开发循环**不是一回事**，不要混淆。

### 五、支持的 Agent 工具

| 工具 | Prompt | Skill | 开发循环 | 特点 |
|------|--------|-------|---------|------|
| **Claude Code** | ✅ | ✅ | ✅ | Query Loop 异步生成器，CLAUDE.md + SKILL.md |
