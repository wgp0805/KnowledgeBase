...
source: "https://paicoding.com/column/14/8"
author:
...
这在 ReAct 模式里特别有用——Agent 每轮循环产生的 intermediate\_steps 需要累积，用 AppenderChannel 就能做到。每次 Agent 调用工具后的结果都会追加到列表里，下一轮 Agent 能看到完整的历史。
...
条件边是 LangGraph4j 最强大的武器。有了它，Agent 可以自己决定“我是直接给答案，还是先调个工具再回来”。这就是 ReAct 模式的图结构表达——Agent 判断任务完成就走到 END，判断需要工具就路由到 tool\_executor 节点，执行完再回来继续思考，形成循环。
...
这就是一个循环，DAG 做不了。还有 ReAct 模式——Agent 判断是否需要调用工具，需要就路由到 tool\_executor，执行完再回来继续思考。
