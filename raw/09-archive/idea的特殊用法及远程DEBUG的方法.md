---
title: idea的特殊用法及远程DEBUG的方法
tags:
  - IDEA
categories:
  - IDEA
date: 2024-06-03 15:11:17
---
...
IDEA可以模拟liunx系统的环境运行，需要在

在 `VM options` 添加 `-Dos.name=linux`
...
高低版本的 IDEA 的设置可能界面有点不一样，我用2023.1.5的。大致上差不多，自行摸索。

IDEA打开远程启动的springboot应用程序所对应的

1.选择 Edit Configuration

![img](2586325914-64e474e6754a4_fix732.png)

2.如图，点击加号，选择Remote 或者 Remote JVM Debug

![img](954939322-64e474e6d693a_fix732.png)

3.配置，详细步骤见图

![img](3821472146-64e474e78f3f4_fix732.png)
...
使用第一步得到的 `Command line arguments for remote JVM` 即可，即`-agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=8088`

改造后的启动脚本如下

```sh
nohup java -Xdebug -agentlib:jdwp=transport=dt_socket,server=y,suspend=n,address=8088 -jar ruoyi-admin.jar 
```
...
**这里需要注意，服务器上的启动端口该是什么就是什么，比如我这里服务器的启动端口是8089，然后在remote中配置的端口是远程调试监听端口8088，启动服务器时如不适用启动脚本，可以直接按照上方的启动指令去启动，服务启动成功后，启动本地的Remote，就可以看到`Connected to the target VM, address: '192.168.0.1:8088', transport: 'socket'`的字样，证明已经链接成功了，就可以在本地打断点进行调试了**

1、端口可随意自己定，未被占用的都行，但是要和IDEA里的remote中设置的端口一致！其他参数照抄。详细的参数解释可以参照附录或自己搜

2、`ruoyi-admin.jar` 改成给你自己的 jar 包名字

3、我给的脚本是后台运行的，如不需要后台运行，自行去掉 `nohup` 和 `&`

3、启动springboot，启动IDEA里的

![img](4227249865-64e474e85e1cd_fix732.png)
...
![img](2356351596-64e474e8c2895_fix732.png)
...
关于`drop frame`的问题，如果`drop frame`了重新进行调试，会不会插入2条记录？

如图 `userMapper.insert(eo)` ，本方法没有使用 `@Transactional` 修饰，mapper方法执行过后事务会被立即提交，则库表里多了一行记录，如果`drop frame`后，再次进行调试，再次执行这代码，于是又插入了一条记录。

![img](1258165925-64e474e96a627_fix732.png)

如果加上 `@Transational` 就不会有两条记录了，dropframe的时候事务没被提交，再次执行该插入代码也不会插入2条。

##### 关于什么是drop frame

![img](3752285921-64e474ea29ce3_fix732.png)
